    ...
    for time_point in time_points:
        for group in groups:
            ...
            for motif in motifs:
                ...
                group_data = df[(df['Time_Point'] == time_point) & (df['Group'] == group) & (df['Motif'] == motif)]
                sham_data = df[(df['Time_Point'] == time_point) & (df['Group'] == 'Sham') & (df['Motif'] == motif)]

                # Check if both distributions are normal
                if group_data['Is_Normal_Group'].all() and sham_data['Is_Normal_Group'].all():
                    test = 't_stat'
                else:
                    test = 'u_stat'

                # Select the appropriate value column based on log_comp
                value_column = 'Log_Normalized_Value' if log_comp else 'Normalized_Value'
                ...
                if not group_values.empty and not sham_values.empty:
                    if test == 't_stat':
                        t_stat, p_val = ttest_ind(group_values, sham_values, equal_var=False, nan_policy='omit')
                        new_row = {
                        ...
                        u_stat, p_val = mannwhitneyu(group_values, sham_values, alternative='two-sided')
                        new_row = {
                            'Animal_ID': group_data['Animal_ID'].iloc[0],
                            'Time_Point': time_point,
                            'Group': group,
    ...
    for time_point in time_points:
        for state in states:
            ...
            for motif in motifs:
                ...
                state_data = df[(df['Time_Point'] == time_point) & (df['Treatment_State'] == state) & (df['Motif'] == motif)]
                sham_data = df[(df['Time_Point'] == time_point) & (df['Treatment_State'] == 'Sham') & (df['Motif'] == motif)]
                ...
                if not state_values.empty and not sham_values.empty:
                    if test == 't_stat':
                        t_stat, p_val = ttest_ind(state_values, sham_values, equal_var=False, nan_policy='omit')
                        new_row = {
                        ...
                        u_stat, p_val = mannwhitneyu(state_values, sham_values, alternative='two-sided')
                        new_row = {
                            'Animal_ID': state_data['Animal_ID'].iloc[0],
                            'Time_Point': time_point,
                            'Treatment_State': state,
...
def calculate_p_values_vs_baseline_Group(df, log_comp=False):
    ...
    for group in groups:
        for motif in motifs:
            ...
            baseline_group_data = df[(df['Time_Point'].isin(['Baseline_1', 'Baseline_2'])) & (df['Group'] == group) & (df['Motif'] == motif)]
            baseline_value_column = 'Log_Normalized_Value' if log_comp else 'Normalized_Value'
            baseline_values = baseline_group_data[baseline_value_column]

            # Iterate over each time point
            for time_point in time_points:
                if time_point in ['Baseline_1', 'Baseline_2'] or (time_point == 'Drug_Trt' and group == 'ABX'):
                    continue
                ...
                # Check if both distributions are normal
                if baseline_group_data['Is_Normal_Group'].all() and group_data['Is_Normal_Group'].all():
                    test = 't_stat'
                else:
                    test = 'u_stat'
                ...
                if not group_values.empty and not baseline_values.empty:
                    if test == 't_stat':
                        t_stat, p_val = ttest_ind(group_values, baseline_values, equal_var=False, nan_policy='omit')
                        new_row = {
                            'Animal_ID': group_data['Animal_ID'].iloc[0],
                            'Time_Point': time_point,
                            'Group': group,
                        ...
                        u_stat, p_val = mannwhitneyu(group_values, baseline_values, alternative='two-sided')
                        new_row = {
                            'Animal_ID': group_data['Animal_ID'].iloc[0],
                            'Time_Point': time_point,
                            'Group': group,
...
def calculate_p_values_vs_baseline_State(df, log_comp=False):
    ...
    for state in states:
        for motif in motifs:
            ...
            if state == "Treated":
                baseline_state_data = df[(df['Time_Point'].isin(['Baseline_1', 'Baseline_2'])) & (df['Treatment_State'] == 'Injured') & (df['Motif'] == motif)]
            else:
                baseline_state_data = df[(df['Time_Point'].isin(['Baseline_1', 'Baseline_2'])) & (df['Treatment_State'] == state) & (df['Motif'] == motif)]
            ...
            # Iterate over each time point
            for time_point in time_points:
                if time_point in ['Baseline_1', 'Baseline_2'] or (time_point == 'Drug_Trt' and state == 'ABX'):
                    continue
                ...
                # Check if both distributions are normal
                if baseline_state_data['Is_Normal_State'].all() and state_data['Is_Normal_State'].all():
                    test = 't_stat'
                else:
                    test = 'u_stat'
                ...
                if not state_values.empty and not baseline_values.empty:
                    if test == 't_stat':
                        t_stat, p_val = ttest_ind(state_values, baseline_values, equal_var=False, nan_policy='omit')
                        new_row = {
                            'Animal_ID': state_data['Animal_ID'].iloc[0],
                            'Time_Point': time_point,
                            'Treatment_State': state,
                        ...
                        u_stat, p_val = mannwhitneyu(state_values, baseline_values, alternative='two-sided')
                        new_row = {
                            'Animal_ID': state_data['Animal_ID'].iloc[0],
                            'Time_Point': time_point,
                            'Treatment_State': state,
