...
def manage_and_save_model(fabric, train_start, epoch, avg_weight, avg_test_mse_loss, model, cfg, convergence, conv_counter, model_name, BEST_LOSS, SNAPSHOT):
                ...
                if avg_weight.item() > 0.99 and avg_test_mse_loss.item() <= BEST_LOSS:
                    ...
                    try:
                        ...
                        logging.info(f"Saving model failed. Error: {e}")
                    convergence = 0
                else:
                    convergence += 1
                ...
                try: 
                    ...
                    if epoch % SNAPSHOT == 0:
                        ...
                        try:
                            ...
                            logging.info(f"Saving model snapshot failed. Error: {e}")
                except Exception as e:
                    logging.info(f"Saving model snapshot failed. Error: {e}")
...
def train_model(config):
    ...
    logging.info("Logging in to wandb...")
    ...
    logging.info("Moving model and optimizer to device...")  
    ...
    for epoch in range(1, EPOCHS):
        ...
        logging.info(f'Epoch: {epoch}, Epochs on convergence counter: {convergence}')
        ...
        logging.info('Train loss: {:.3f}, MSE-Loss: {:.3f}, MSE-Future-Loss {:.3f}, KL-Loss: {:.3f}, Kmeans-Loss: {:.3f}, weight: {:.2f}'.format(avg_train_loss.item(),
            avg_train_mse_loss.item(), avg_train_fut_loss.item(), avg_train_kl_loss.item(), avg_train_km_loss.item(), avg_weight.item()))
        
        logging.info('Test loss: {:.3f}, MSE-Loss: {:.3f}, KL-Loss: {:.3f}, Kmeans-Loss: {:.3f}'.format(avg_test_loss.item(),
        ...
        logging.info(f"Continuing to next epoch... {epoch+1}, global rank: {fabric.global_rank}")
