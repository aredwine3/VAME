...
def train_model(config, wandb_usage='n'):
    if isinstance(config, dict) and 'learning_rate' in config:
        # Hyperparameters will be directly provided by wandb during sweeps
        ...
        # Read from your own config file if wandb sweeps are not used
...
wandb_usage = input("Do you want to use wandb for logging (l), sweeps (s), or neither (n)? ").lower()
...
if wandb_usage == 's':
    ...
    wandb_sweep_count = int(input("Please enter the number of sweeps you want to run: "))
    wandb.login(key=wandb_api_key)

    wandb_sweep_config = input("Please enter the path to your wandb sweep config file: ")

    # Read the configuration file at the path given by wandb_sweep_config
    with open(wandb_sweep_config, 'r') as f:
    ...
    wandb.agent(sweep_id, lambda: train_model(None, wandb_usage='s'), count=wandb_sweep_count)
    ...
    train_model(config_file_path, wandb_usage='l')
    
else:
    # No wandb, read hyperparameters from your own config file
    config_file_path = input("Enter the path to your config file: ")
    train_model(config_file_path, wandb_usage='n')
