...
class VAE(L.LightningModule):
    def __init__(self, hparams):
        ...
        self.model = RNN_VAE(
            self.hparams["temporal_window"],
            self.hparams["zdims"],
            self.hparams["num_features"],
            self.hparams["future_decoder"],
            self.hparams["prediction_steps"],
            self.hparams["hidden_size_layer_1"],
            self.hparams["hidden_size_layer_2"],
            self.hparams["hidden_size_rec"],
            self.hparams["hidden_size_pred"],
            self.hparams["dropout_encoder"],
            self.hparams["dropout_rec"],
            self.hparams["dropout_pred"],
            self.hparams["softplus"],
        )
    ...
    def training_step(self, batch, batch_idx, future_decoder):
        ...
        data = data_item[:, : int(self.hparams["temporal_window"] / 2), :].type(
            "self.device"
        )
        fut = data_item[
            :,
            int(self.hparams["temporal_window"] / 2) : int(
                self.hparams["temporal_window"] / 2
            )
            + self.hparams["prediction_steps"],
            :,
        ].type("self.device")
        ...
        if self.hparams["future_decoder"]:
            ...
            kl_weight = kl_annealing(
                self.current_epoch, self.kl_start, self.annealtime, self.anneal_function
            )
            loss = (
                rec_loss
                + fut_rec_loss
    ...
    def training_epoch_end(self, outputs):
        ...
        print(
            "Train loss: {:.3f}, MSE-Loss: {:.3f}, KL-Loss: {:.3f}, Kmeans-Loss: {:.3f}, weight: {:.2f}".format(
                avg_loss,
                torch.stack(self.mse_losses).mean(),
                self.BETA * self.weight_values[-1] * torch.stack(self.kl_losses).mean(),
                self.weight_values[-1] * torch.stack(self.kmeans_losses).mean(),
    ...
    def configure_optimizers(self):
        ...
        if self.optimizer_scheduler:
            self.scheduler = ReduceLROnPlateau(
                self.optimizer,
                "min",
                factor=self.hparams["scheduler_gamma"],
                patience=self.hparams["scheduler_step_size"],
                threshold=self.scheduler_thresh,
                threshold_mode="rel",
                verbose=True,
            )
        else:
    ...
    def val_dataloader(self):
        self.test_loader = Data.DataLoader(
            self.testset,
            batch_size=int(self.hparams["batch_size"] / 4),
            shuffle=True,
            drop_last=True,
            num_workers=4,
        )
    ...
    def prepare_data(self):
        self.trainset = SEQUENCE_DATASET(
            os.path.join(self.hparams["project_path"], "data", "train", ""),
            data="train_seq.npy",
            train=True,
...
callbacks = [
    ...
    ModelCheckpoint(filepath="model-{epoch:02d}-{val_loss:.2f}"),
]
